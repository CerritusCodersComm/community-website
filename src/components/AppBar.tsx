  ...
  return (
    ...
    >
      ...
      <div
        className={
          !nav
            ? "fixed left-0 top-0 w-[100%] h-screen border-r border-r-grey-900 ease-in-out duration-700 bg-lightPurple"
            : "fixed left-[-100%] ease-in-out border-r border-r-grey-900 w-[100%] bg-lightPurple duration-700 h-screen"
        }
      >
        <div className="mt-2 ml-10">
          <Image src={cerritusCodersLogo} height={107} width={214} />
        ...
        <div className="text-2xl flex-shrink-0 pt-0 px-10 flex-cols bg-lightPurple">
          {navLinks.map((link, index) => {
            return (
              <div
                className="flex justify-center space-x-1 m-2 p-2 cursor-pointer"
                key={index}
                onClick={() => setNav((prev) => !prev)}
              >
                <ul>
